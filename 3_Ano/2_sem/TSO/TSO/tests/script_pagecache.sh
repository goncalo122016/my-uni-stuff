#!/bin/bash
# script_pagecache.sh
# Testa o sistema de deduplicação com I/O buffered (sem --direct=1)
# para avaliar o impacto da page cache do FUSE (user-space LRU) combinada
# com o page cache do kernel.
...
for DEDUPE in 0 50 100; do
    ...
    sudo $BPF_DIR/systracer $FUSE_PID > $RESULTS_DIR/pc_bpf_${DEDUPE}.txt 2>&1 &
    BPF_PID=$!
    sleep 1

    # Iniciar monitorização de recursos (CPU e Memória) do processo FUSE
    pidstat -p $FUSE_PID -r -u 1 > $RESULTS_DIR/pc_fuse_resources_${DEDUPE}.txt &
    PIDSTAT_PID=$!

    # Correr FIO — SEM --direct=1 para aproveitar o page cache do kernel
    fio --name=pc_dedupe_${DEDUPE} \
        --directory=$MOUNT_POINT \
        --rw=write --bs=4k --size=100m \
        --ioengine=psync \
        --dedupe_percentage=$DEDUPE \
        --output=$RESULTS_DIR/pc_dedupe_${DEDUPE}.json \
        --output-format=json
    ...
    # Processar os resultados do BPF
    echo "Resumo de I/O BPF (Layer 0=Syscall, Layer 2=EXT4):" >> $RESULTS_DIR/pc_disk_usage.txt
    awk 'NR>1 {
    ...
    }' $RESULTS_DIR/pc_bpf_${DEDUPE}.txt >> $RESULTS_DIR/pc_disk_usage.txt

    echo "=== Resultado ${DEDUPE}% concluído ==="
    sleep 2
done

echo "Todos os testes Page Cache concluídos! Resultados em $RESULTS_DIR"
...
echo "Blocos únicos manuais: $(ls /backend/.dedupblocks/ | wc -l)" >> $RESULTS_DIR/pc_disk_usage.txt
echo "Espaço usado manual: $(du -sh /backend/.dedupblocks/)" >> $RESULTS_DIR/pc_disk_usage.txt
