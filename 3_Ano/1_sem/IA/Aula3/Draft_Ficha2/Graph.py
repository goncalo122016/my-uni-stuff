...
class Graph:
    ...
    def procura_DFS(self, start, goal, path, visited):
        path = path + [start]
        visited.add(start)
        ...
        for (neighbor, weight) in self.getNeighbours(start):
            if neighbor not in visited:
                new_path = self.procura_DFS(neighbor, goal, path, visited)
                if new_path:
                    return new_path
        return None
    ...
    def procura_BFS(self, start, goal):
        visited = set()
        queue = Queue()
